...
Функция ОтправитьHTTPЗапрос(Сессия, ПодготовленныйЗапрос, Настройки)
	...
	Если ДанныеПонастройкамИнтеграции <> Неопределено Тогда 
		Если ДанныеПонастройкамИнтеграции.Количество() > 0 Тогда 
			РегистрыСведений.ЛогиЗапросов.СоздатьЗаписьЛога(ПодготовленныйЗапрос.HTTPЗапрос, ОтветСтрокой,  ПодготовленныйЗапрос.Метод, ДанныеПонастройкамИнтеграции, ДлительностьЗапроса, Ответ);
		КонецЕсли;
	КонецЕсли;
